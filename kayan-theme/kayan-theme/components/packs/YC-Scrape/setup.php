...
class YourColor__ScrapePlugin{
	...
	public function FormatString($string) {
		...
		$string = trim($string);
		if( mb_substr($string, 0, 1) == ' ' ) $string = mb_substr($string, 1);
		if( mb_substr($string, (mb_strlen($string) - 1), 1) == ' ' ) $string = mb_substr($string, 0, (mb_strlen($string) - 1));
		return $string;
	}
	public function HtmlBeautify($html){
	...
	public function GetDivByAttr($string,$attr_key,$attr_value){	
		$Return = array();
		$tidy = new tidy();
	    ...
	    $s = new DOMXPath($d);
	    foreach( $s->query('//*[contains(attribute::'.$attr_key.', "'.$attr_value.'")]') as $node ){
	        $target = $node->ownerDocument->saveHTML($node);
			$target = html_entity_decode($target);	
			$target = $tidy->repairString($target);	
			$target = $this->HtmlBeautify($target);
			$target = explode('<body>', $target)[1];
			$target = explode('</body>', $target)[0];
			$Return[] = $target;
    	}	    

    	return $Return;
	}
	public function RemoveDivByAttr($string,$attr_key,$list=array() ){
		$tidy = new tidy();
		libxml_use_internal_errors(true);
	    $d = new DOMDocument();
	    $d->loadHTML( mb_convert_encoding($string, 'HTML-ENTITIES', 'UTF-8') );
	    $s = new DOMXPath($d);
	    foreach ( $list as $k => $remove__item) {
		    foreach( $s->query('//*[contains(attribute::'.$attr_key.', "'.$remove__item.'")]') as $t ){
		        $t->parentNode->removeChild($t);
	    	}
	    }

	    $FinalHTML =  $d->saveHTML();
		$FinalHTML = html_entity_decode($FinalHTML);	
		$FinalHTML = $tidy->repairString($FinalHTML);	
		$FinalHTML = $this->HtmlBeautify($FinalHTML);
		$FinalHTML = explode('<body>', $FinalHTML)[1];
		$FinalHTML = explode('</body>', $FinalHTML)[0];
	    return $FinalHTML;
	}

	public function RemoveAttirbutes( $html ){
		$dom = new DOMDocument();
		$tidy = new tidy();
		libxml_use_internal_errors(true);
		...
		foreach ($elements as $element) {
		    for ($i = $element->attributes->length; --$i >= 0;) {
		        ...
		        if (('img' === $element->nodeName && 'src' === $name)
		            ...
		            || ('iframe' === $element->nodeName && 'src' === $name)
		            || ('iframe' === $element->nodeName && 'width' === $name)
		            || ('iframe' === $element->nodeName && 'height' === $name)
		        ) {
		            continue;
		        }
...
(new YourColor__ScrapePlugin)->Setup();