...
if( !isset($_GET['ajax']) ) {
	...
	echo '<head>';
		...
		// ═══ KAYAN — meta description: Rank Math يبقى المرجع الوحيد عند وجوده ═══
		// Rank Math يطبع meta description صحيحة لكل صفحة عبر wp_head() — لا نتدخل يدوياً.
		// نطبع وصفاً احتياطياً فقط عند غياب Rank Math تماماً (توافق رجعي مع مواقع بلا RM).
		...
		// ═══ KAYAN — Title Management ═══
		// Rank Math موجود → wp_head() يُخرج <title> تلقائياً (add_theme_support('title-tag') في theme-seo)
		// Rank Math غائب → ThemeSeo()->Title() تُخرجه مباشرةً (fallback فقط)
		$hide__theme_seo = get_option('hide__theme_seo');
		$yc__rankmath_active = class_exists( 'RankMath' ) || class_exists( 'RankMath\\RankMath' );
		if ( empty( $hide__theme_seo ) && ! $yc__rankmath_active && class_exists( 'ThemeSeo' ) ) {
			(new ThemeSeo)->Title();
		}
		...
		echo '<link rel="stylesheet" href="'.esc_url( get_template_directory_uri().'/components/styles/fa-free-fixes.css?v=2.4.1' ).'" />';
